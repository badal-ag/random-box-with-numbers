      ...
      .then(function() {
        return new Promise(function(res, rej) {
          that.clearId.push(setTimeout(function() {
            const s = new Shape(that.ctx, that.width / 2, that.height * (Math.random() + Math.random()) / 2, i);
            that.shapes.push(s);
            res();
          }, 240));
        });
...
class Shape {
  ...
  wrapPosition() {
    if (this.x > canvas.width) this.init(canvas.width / 2, Tool.randomNumber(0, canvas.height), this.i);
    if (this.x + this.r < 0) this.init(canvas.width / 2, Tool.randomNumber(0, canvas.height), this.i);
  }
